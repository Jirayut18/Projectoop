...
export default function All_csv() {
  ...
  function get_data() {
    ...
    const csv_predict = data3.map((item) => item.PredictionString);

    const csv_id = data3.map((item) => item.id);

    //split in id string "_"
    const csv_id_split = csv_id.map((item) => item.split("_"));
    ...
    //split " "
    const csv_predict_split = csv_predict.map((item) => item.split(" "));
  ...
  return (
    <div className="border">
      <button onClick={get_data} className="btn btn-outline-success">
        Sample_Submission
      </button>
      <div className="csv">
